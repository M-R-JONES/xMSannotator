multilevelannotationstep3 <- function(outloc1, adduct_weights = NA, num_sets = NA, boostIDs = NA, pathwaycheckmode = "p", 
  ...
  load("chemCompMZ.Rda") #all possible forms for each item in the database (nrows is defined by number of adducts considered)
  ...
  #set the directory to match that used for storing chemical annotation .rda objects
  outloc1 <- paste(outloc, "/stage2/", sep = "")
  suppressWarnings(dir.create(outloc1))
  setwd(outloc1)
  
  #load the saved annotation dataframe for each chemid
  chemscoremat <- lapply(1:num_sets, function(sind) {
      cur_fname <- paste("chem_score", sind, ".Rda", sep = "")
      
      #try to load a .rda object associated with the particular chemid
      try(load(cur_fname), silent = TRUE)
      
      curchemscoremat <- as.data.frame(curchemscoremat)
      
      #removes all indication of isotope forms, e.g. M+H_[+1] become M+H
  ...
  #merge list outputs in to data frame
  chemscoremat <- ldply(chemscoremat, rbind)
  chemscoremat <- as.data.frame(chemscoremat)
  
  #extract all adduct forms from the chemscoremat object and then remove all information relating to isotope assignment
  tempadduct <- chemscoremat$Adduct
  chemscoremat$Adduct <- gsub(chemscoremat$Adduct, pattern = "_.*",replacement = "")
  
  #merge chemical score matrix with the adduct information
  chemscoremat <- cbind(chemscoremat, tempadduct)
  
  #merge dataframes (updated to use explicit column names)
  chemscoremat <- merge(chemscoremat, 
                        data.frame('mz' = chemCompMZ$mz, 'Name' = chemCompMZ$Name, 'Adduct' = chemCompMZ$Adduct), #chemCompMZ[, c(2:4,6)], 
                        by = c("Formula", "Adduct"))
  
  
  chemscoremat = data.frame('chem_score' = chemscoremat$chemical_score,
                            'Module_RTclust' = chemscoremat$Module_RTclust ,
                            "mz" = chemscoremat$mz,
                            'time' = chemscoremat$time,
                            'MatchCategory' = chemscoremat$MatchCategory,
                            'theoretical.mz' = chemscoremat$theoretical.mz,
                            'chemical_ID' = chemscoremat$chemical_ID.y,
                            'Name' = chemscoremat$Name,
                            'Formula' = chemscoremat$Formula,
                            'MonoisotopicMass' = chemscoremat$MonoisotopicMass,
                            'tempadduct' = chemscoremat$tempadduct,
                            'ISgroup' = chemscoremat$ISgroup,
                            'AvgIntensity' = chemscoremat$AvgIntensity,
                            'MD' = chemscoremat$MD)
