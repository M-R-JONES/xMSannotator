get_chemscorev1.6.71_custom = function(chemicalid = chemid,
  ...
  if(length(mchemicaldata$mz)>0){
    ...
    #  then retain this feature in further evaluation steps.
    ...
    #load("~/temp/outputs/env_start_of_scoring.rdata")
      ...
      for(i in 1:length(which(table_mod>=1))){
        ...
        if(length(mchemicaldata$mz)>=1){ 
          ...
          if(chemical_score>best_chemical_score){
            ...
            best_chemical_score<- as.numeric(assigning.score$chemical_score)
            best_conf_level<-1
            best_mod_ind<-i
            best_data<-mchemicaldata
            
          }else if (as.numeric(assigning.score$chemical_score)==best_chemical_score){
            ...
            if(diff_rt<=max_diff_rt){
              ...
              if(dim(mchemicaldata)[1]>1){
                ...
                assigning.score = calc_adduct_isotope_score(k_power = 1, adduct_weights=adduct_weights, mchemicaldata = mchemicaldata,
                                          topquant_cor, calc_iso_score = T, adduct_scoring_method = 'simple')
              ...
              #THIS STEP APPLIES WHEN PEAKS SHARE THE SAME FORMULA ASSIGNMENT, RTclust module, MASS DEFECT, but their retention time differ by
              #   user-defined max_diff_rt parameter (e.g. perhaps due to shift in retention times?)
              ...
              #determine which adducts are associated with the top_mod_sub (includes isotopes assigned as adduct, e.g. M+H_[+1] which becomes M+H)
              cur_adducts_with_isotopes<-mchemicaldata$Adduct
              cur_adducts<-gsub(cur_adducts_with_isotopes,pattern="(_\\[(\\+|\\-)[0-9]*\\])",replacement="")
              ...
              if(diff_rt>=2*iqr1){
                ...
                time_cor_groups<-sapply(list(myData1=mchemicaldata),function(x){
                  split(x,cut(mchemicaldata$time,breaks=seq(min_val-iqr1,max_val+iqr1,2*iqr1)))
                  #earlier version
              ...
              #group_ind_val<-1 #not used for anything
              #group_ind_size<-1
              check_reladd<-{}
              check_data<-{}
              ...
              #  group_ind_val<-which(group_sizes==max(group_sizes)[1]) #if two subsets have the same number of features, two or more values could be returned
              #}
              ...
              #### this section doesn't factor in the scoring of each individual group, just the group with the largest number of features
              # furthermore, the mchemicaldata is subsetted using top_mod[i] - this would one again reintroduce the issue of multiple mis-aligned peaks being processed together
              # #if fewer than 2 features in the largest group
              # if(group_ind_size<2){
              #   
              #   # as before, now calculate chemical score 
              #   # note, no isotope-based contribution to the score (we have n = 1 line, hence no isotopes)
              #   k_power<-1.25
              #   
              #   mchemicaldata<-mchemicaldata_orig[which(mchemicaldata_orig$Module_RTclust==top_mod[i]),]
              #   mchemicaldata<-mchemicaldata[order(mchemicaldata$mz),]
              ...
              #an empty contain that is used for storing the index of peak groups that have at least one valid adduct
              good_temp<-{}
              ...
              if(length(which(group_sizes>1))>0){
                ...
                for(g2 in 1:length(group_sizes)){
                  ...
                  if(g2%in%good_temp){
                    ...
                    diff_rt<-abs(max(as.numeric(mchemicaldata$time)) - min(as.numeric(mchemicaldata$time)))
                    diff_rt<-round(diff_rt)
                    
                    #creates empty object and then asks if the length is greater than 1....pointless
                    #dup_add<-{} #which(duplicated(mchemicaldata$Adduct)==TRUE)
                    #if(length(dup_add)>0){
                    #  dup_data<-mchemicaldata[c(dup_add),]
                    #  #mchemicaldata<-mchemicaldata[-c(dup_add),]
                    ...
                    #if, within this group of peaks, the retention time range is less-than the max_diff_rt defined by user
                    if(diff_rt<=max_diff_rt){

                      #if at least two rows in the group of features
                      if(dim(mchemicaldata)[1]>1){
                        
                        #for features in group, assign the chemical score to constituents
                        k_power<-1
                        ...
                        if(good_adducts_len>0){
                          ...
                          chemical_score<-sum(chemical_score*(10^max(as.numeric(as.character(adduct_weights[which(adduct_weights$Adduct%in%cur_adducts_with_isotopes),2])))))
                        ...
                        #when retention times within group still exceed the max_diff_rt, then set score of 0
                        chemical_score<-0
                      }
                      
                      names(chemical_score)<-as.character(chemicalid[1])
